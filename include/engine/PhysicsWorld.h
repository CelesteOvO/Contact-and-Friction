...
public:
    ...
    void setPreStepFunc(PreStepFunc _func) { m_preStepFunc = std::move(_func); }
    void setResetFunc(ResetFunc _func) { m_resetFunc = std::move(_func); }
...
private:
    ...
    void calcConstraintForces(float dt); /// 计算约束力
...
public:
    ...
    PreStepFunc m_preStepFunc;      /// 用于在每一步模拟之前调用的函数
    ResetFunc m_resetFunc;          /// 用于重置系统的函数
};
